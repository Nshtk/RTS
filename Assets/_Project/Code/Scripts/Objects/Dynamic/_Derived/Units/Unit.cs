...
public partial class Unit : DynamicObject
{
	...
	public class UnitController
	{
		...
		private void turn()
		{
			_unit.transform.rotation=Quaternion.RotateTowards(_unit.transform.rotation, rotation_direction, speed_rotate_max*Time.deltaTime);
		}
		public virtual void accelerate(Vector3 direction, float speed, float speed_max)	//TODO separate to accelerateForward()
		{
			if (!_unit.Is_Grounded)		//TODO To GroundUnit 
				return;
			_unit._rigid_body.AddForce(direction*speed);   //REVIEW * Time.fixedDeltaTime?
		...
		public virtual void move()
		{
			...
			Debug.DrawRay(_unit.transform.position, _unit.transform.forward*3, Color.blue, 0f, true);

			if (Vector3.Dot(_unit.transform.up, Vector3.down)>0)  //Reset flipped object
			{
				_unit.transform.up=Vector3.up;
				_unit.transform.forward=direction;
			}

			var a = Vector3.Angle(direction, _unit.transform.forward);
			if (Vector3.Angle(new Vector3(direction.x, 0, direction.z), new Vector3(_unit.transform.forward.x, 0, _unit.transform.forward.z))>10)        //(Quaternion.Angle(Quaternion.LookRotation(new Vector3(_unit.transform.rotation.x, 0, _unit.transform.rotation.z)), rotation_direction)>0
			{
				turn();
				if (_unit.movement_type==UNIT_MOVEMENT_TYPE.TRACKED)
					return;
			}

			accelerate(_unit.transform.forward, speed_move, speed_move_max);
			_waypoint.updateRemainingDistance(_unit.transform.position);
		}
		public void stop()
		{
			if ((_unit.transform.forward * Vector3.Dot(_unit.transform.forward, _unit._rigid_body.velocity)).magnitude>0)
				accelerate(-_unit.transform.forward, speed_stop, speed_stop_max);//_unit._rigid_body.AddForce(-_unit.transform.forward*speed_stop);
		}
